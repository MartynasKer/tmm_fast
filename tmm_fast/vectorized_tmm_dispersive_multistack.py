...
def coh_vec_tmm_disp_mstack(
    ...
    * pol, n_list, d_list, th_0, lam_vac--same as input
    ...
    theta = (
        2 * np.pi * torch.einsum("skij,sij->skij", cos(SnellThetas), N)
    )  # [theta,d, lambda]
    ...
    M_list[:, :, :, 1:-1, 0, 0] = torch.einsum(
        "shji,sjhi->sjhi", 1 / A, 1 / t_list[:, :, :, 1:]
    )
    M_list[:, :, :, 1:-1, 0, 1] = torch.einsum(
        "shji,sjhi->sjhi", 1 / A, F / t_list[:, :, :, 1:]
    )
...
def SnellLaw_vectorized(n, th):
    ...
    assert n0th.shape == (n.shape[0], n.shape[-1], th.shape[0]), (
        n.shape[-1],
        th.shape[0],
    )
    angles = asin(torch.einsum("sij,ski->sjki", n0th, 1 / n))
    assert angles.shape == (n.shape[0], th.shape[0], n.shape[1], n.shape[2]), (
        th.shape[0],
        n.shape[0],
        n.shape[1],
    )
...
def is_not_forward_angle(n, theta):
    ...
    answer = (~(abs(ncostheta.imag) > EPSILON)) * (ncostheta.real > -1 * EPSILON)
    ...
    assert (ncostheta.imag > -1 * EPSILON)[answer].all(), (
        error_string + "; first assert"
    )
    assert (ncostheta.real > -1 * EPSILON)[answer].all(), (
        error_string + "; second assert"
    )

    assert ((n * cos(torch.conj(theta))).real > -1 * EPSILON)[answer].all(), (
        error_string + "; third assert"
    )
    assert (ncostheta.imag < EPSILON)[~answer].all(), error_string + "; fourth assert"
    if not (ncostheta.real < EPSILON)[~answer].all():
        print(ncostheta[~answer].imag)
    assert (ncostheta.real < EPSILON)[~answer].all(), error_string + "; fifth assert"
    assert ((n * cos(torch.conj(theta))).real < EPSILON)[~answer].all(), (
        error_string + "; sixth assert"
    )
...
def is_forward_angle(n, theta):
    ...
    if answer is True:
        ...
        assert torch.all(ncostheta.real < 100 * EPSILON), error_string
        assert torch.all(
            (n * cos(theta.conjugate())).real < 100 * EPSILON
        ), error_string
    return answer
...
def converter(data: Union[np.ndarray, torch.Tensor], device: str) -> torch.Tensor:
    ...
    if type(data) is not torch.Tensor:
        if type(data) is np.ndarray:
            ...
            raise ValueError(
                "At least one of the inputs (i.e. N, Theta, ...) is not of type numpy.array or torch.Tensor!"
            )
...
def check_datatype(N, T, lambda_vacuum, Theta):
    assert type(N) == type(T) == type(lambda_vacuum) == type(Theta), ValueError(
        "All inputs (i.e. N, Theta, ...) must be of the same data type, i.e. numpy.array or torch.Tensor!"
    )
    return type(N)
...
def check_inputs(N, T, lambda_vacuum, Theta):
    ...
    assert T.shape[0] == N.shape[0] and T.shape[1] == N.shape[1], (
        "First and second dimension of N and T must coincide,\nfound N.shape="
        + str(N.shape)
        + " and T.shape="
        + str(T.shape)
        + " instead!"
    ...
    assert (
        Theta.ndim == 1
    ), "Theta is not of shape [A] (1d), as it is of dimension " + str(Theta.ndim)
    # check the dimensionality of lambda_vacuum:
    assert (
        lambda_vacuum.ndim == 1
    ), "lambda_vacuum is not of shape [W] (1d), as it is of dimension " + str(
        lambda_vacuum.ndim
    )
    assert N.shape[-1] == lambda_vacuum.shape[0], (
        "The last dimension of N must coincide with the dimension of lambda_vacuum (W),\nfound N.shape[-1]="
        + str(N.shape[-1])
        + " and lambda_vacuum.shape[0]="
        + str(lambda_vacuum.shape[0])
        + " instead!"
    )
    # check non-imaginary property of refractive indicies for the first and last layer:
    answer = torch.any((N[:, 0, :].imag > EPSILON))
    assert (
        not answer
    ), "Non real-valued refractive indicies detected for first layer, i.g. N[:, 0, w].imag > 0 for some valid w!"
    answer = torch.any((N[:, -1, :].imag > EPSILON))
    assert (
        not answer
    ), "Non real-valued refractive indicies detected for last layer, i.g. N[:, -1, w].imag > 0 for some valid w!"
